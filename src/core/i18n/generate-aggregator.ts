...
export function generateAggregator(
  ...
  if (!fs.existsSync(baseMessagesDir)) {
    console.log(chalk.yellow("⚠️  Messages directory does not exist, skipping aggregator generation"));
    return;
  }
  ...
  if (filesByLocale.size === 0) {
    console.log(chalk.yellow("⚠️  No JSON files found, skipping aggregator generation"));
    return;
  }
