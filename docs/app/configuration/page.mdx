...
i18nizer now includes comprehensive documentation for advanced i18n patterns:

- **Pluralization**: Full guide for ICU message format with plural rules (zero, one, two, few, many, other)
- **Rich Text Formatting**: Patterns for handling JSX elements within translations using `t.rich()` and Trans components
- **Integration**: Works seamlessly with i18next and next-intl built-in features

See the [Examples page](/examples) for detailed pluralization and rich text patterns.
